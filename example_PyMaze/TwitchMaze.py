...
class Game:
  ...
  def __init__(self, diff, dim, path, tPlays):
    self.tPlays = tPlays
    self.size = (800,600)
    ...
    self.diff_start = diff
    self.diff = diff
    self.path_start = path
    self.dont_quit = True
    
    self.dim = list(map(int, dim.split('x')))

  def start(self):
    #self.diff = self.diff_start
  ...
  def loop(self):
    ...
    while self.keep_going and self.dont_quit:
      moved = 0
      self.clock.tick(10)
      # react to keyboard
      ...
      # react to twitch plays
      result = self.tPlays.vote_result()
      if result:
        if result == 's':
          self.move_player('d')
        if result == 'w':
